...
全文分三部分：第 2、3 节说明 Agent SDK 与 `claude -p` 的关系、认证与计费的前提；第 4 至 9 节是实现本身（架构、`query()` 参数、事件流、权限、固定前缀、提示缓存）；第 10 至 16 节是运行期的事项（登录态、定时任务的分发、部署位置、上下文窗口、同一 session 上更换模型、故障对照、版本）。
...
- 上限 12 次。缓存写入价格为读取的 12.5 倍（写 1.25、读 0.1），超过 12.5 次保活的费用高于一次重写。这项机制在费用上接近持平，观察到的效果是长时间空闲后第一轮的等待从约三分钟降到秒级。
- **该效果的因果关系未验证。** 上面的保活请求与对话轮次的请求在系统提示上并不相同：保活不加载 settings、不带系统提示追加段、工具集为空，也不挂载 MCP。2026-09-18 的一次对照（同一 session 上先后两次 `forkSession`，中间无其他请求）：按对话轮次的选项发出的一次 `cache_creation` 502,544；紧接着按保活选项发出的一次 `cache_read` 0、`cache_creation` 489,659，两者相差 12,885 token。差异位于前缀开头，按前缀匹配的规则，其后的对话历史无法复用。据此，保活写入的是另一份前缀，而非对话轮次所用的那一份；上述时延观察与「保活为对话轮次续命」之间的联系尚待测量。可用的验证方式之一是停止保活一夜，比较次日第一轮的 `cache_read`。
...
**窗口以实际拒绝为准。** 订阅侧 Opus 4.6 的裸 id 上限为 200k：一条 thread 在 19.5 万 token 时被拒绝「Prompt is too long」，且该模型此时无法完成 `/compact`；`claude-opus-4-6[1m]` 变体实测 236k 一次通过。Fable 5.1 原生 1M，裸 id 实测 257k 通过。接近上限时的处理：切换到窗口更大的模型对同一 session 发送 `/compact`（session id 不变），完成后切回；这一做法的缓存代价见第 14 节。斜杠命令作为普通 prompt 发送，由 CLI 识别。
...
**模型与 session 是两个独立参数。** `query()` 每轮都接受 `model` 与 `resume`，session id 不随模型变化；同一条 session 的下一轮换用另一个模型，CLI 与上游都不拒绝。前端的模型切换器因此不必为每个模型各维护一条 session。本实现中前端与网关之间只传别名，别名到 id 的表在 daemon 一处（第 5 节）。

**代价在提示缓存，不在 session。** 提示缓存按模型隔离。换到另一个模型的第一轮整段前缀写入（`cache_creation` 约等于上下文大小，`cache_read` 为 0），第二轮起在新模型上照常命中。2026-09-18 的一条演练线（约 8 万 token，工具集为空）：

```
opus 4.6  · ctx 80,795 (cache_read 80,606)
fable 5.1 · ctx 64,652 (cache_read      0)   ← 换模型的这一轮
opus 4.6  · ctx 81,881 (cache_read 80,792)   ← 换回，命中一小时内写入的那一份
```

同一系列的两代（例如 5 与 5.1）是两个模型，同样重写。这几轮的 session id 不变。

**一小时内换回仍然命中。** 缓存一小时过期，旧模型那一份不因为切走而失效。上面第三轮即是：切走一次再切回，`cache_read` 恢复。因此第 13 节的做法——用窗口更大的模型对同一 session 执行 `/compact`、完成后切回——在一小时内只付一次重写。

**思考档位不进缓存键。** 同一模型更换 `effort` 不触发重写（2026-08-31 交叉验证）。

**窗口随模型变化，可在启动子进程前判断。** 同一条 session 在一个模型上装得下，换到窗口较小的模型可能被上游拒绝「Prompt is too long」，这一轮的进程启动与等待没有产出。本实现在 daemon 侧维护一张「模型 id → 已实测窗口」的表，表中只填有依据的值（模型自身报告的上限，或已经通过的最大上下文），无依据的留空。启动子进程前比较转写末轮的上下文读数与该窗口：

```js
const reading = tailReading(transcriptOf(sid));   // { ctxTok, modelId }
const window = WINDOWS[realId] ?? null;           // 无依据时为 null
if (window && reading.ctxTok > window * 0.97) {
  // 不启动子进程，直接返回一条说明；0.97 留给这一轮的输入与工具结果
}
const switched =
  reading.modelId && family(reading.modelId) !== family(realId);
```
